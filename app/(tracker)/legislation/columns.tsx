      ...
      const documentMenuItems: JSX.Element = (
        <>
          {Array.from(documentGroups.entries()).map((entry) => {
            return (
              <DropdownMenuSub
                key={`${legislation.biennium}${legislation.billNumber}${entry[0]}`}
              >
                <DropdownMenuSubTrigger>{entry[0]}</DropdownMenuSubTrigger>
                <DropdownMenuPortal>
                  <DropdownMenuSubContent>
                    <ScrollArea classNameViewport="max-h-[200px] max-w-[350px]">
                      ...
                      {entry[1]
                        ?.sort(
                          (a, b) =>
                            new Date(b.pdfLastModifiedDate).getTime() -
                            new Date(a.pdfLastModifiedDate).getTime()
                        )
                        .map((doc) => (
                          <DropdownMenuItem
                          ...
                          >
                            <Link
                              ...
                              target="_blank"
                              href={doc.pdfUrl}
                            >{`${new Date(doc.pdfLastModifiedDate).toLocaleDateString()}`}</Link>
          ...
          {legislation?.fiscalNotes
            ...
            .map((fiscalNote) => {
              return (
                ...
                >
                  <Link
                    className="min-w-min text-blue-600"
                    passHref={true}
                    target="_blank"
                    href={fiscalNote.fiscalNotePDFUrl}
                  >
